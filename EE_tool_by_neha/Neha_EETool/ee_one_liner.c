...
void findSUsDraw(SaImmHandleT immHandle,SaUint8T *ch,int SGcounter);
void findSIsDraw(SaImmHandleT immHandle,SaUint8T *ch,int SGcounter);
...
GtkWidget *elasticityresultlabel1;
GtkWidget *buffer_actionlabel2;
GtkWidget *win;
GtkWidget *scrolled_window;
GtkWidget *fixed;
GtkWidget *buttonSU[20][20],*buttonSI[20][20];
int finalSU=0,finalSG=0,finalSI=0;
...
char nameSI[256];
//int elasticityact;
int flag_set=3;     		//signal from monitor 
int redrawSUs=0;                       
...
{ 
  ...
  { 
      for(j=0;j<finalSU;j++)
      {
      gtk_widget_modify_bg (GTK_WIDGET(buttonSU[i][j]), GTK_STATE_NORMAL, NULL);
      gtk_object_set(GTK_OBJECT(buttonSU[i][j]), "label", " ", NULL);
      }
  }
}
...
void drawSUs()
{
   ...
   strcpy(objectstr,"safSg=");
    ...
    SaNameT rootName = { 0, "" };
    strncpy((char *)rootName.value,"", SA_MAX_NAME_LENGTH);
                   ...
                   ch =strstr(objectName.value,objectstr);
                    ...
                    if(ch == NULL)
                      ...
                      else
                      {
                          ...
                          if(pos == 0)
                          {
                                 ch1 =strstr(objectName.value,"safApp=OpenSAF");
                                 if(ch1 == NULL)
                                 {
                                 	SGcounter++;
                                 	printf("\n%s",ch);
                                 	findSUsDraw(immHandle,ch,SGcounter);
                                 	ch =strstr(objectName.value,"safApp=");
                                 	findSIsDraw(immHandle,ch,SGcounter);
                          	 }
...
/**callback press event**/
void button_press(GtkWidget *widget,GdkEventButton *event, int data)
{
 int i=0,result1=0;
  if(event->button ==1)
  {
      //printf("\nLeft Click");
      printf("\nElasticity Action: Decrease");
      elasticityact=2; 			//2 is for decrease
     
  }
  if(event->button ==3)
  {
     //printf("\nRight Click");
     printf("\nElasticity Action: Increase");
     elasticityact=1; 			//1 is for increase
  }
	g_print("\n%s was pressed and Elasticity Type %d",gtk_button_get_label(GTK_BUTTON(widget)),data);
        strncpy(nameSI,gtk_button_get_label(GTK_BUTTON(widget)),SA_MAX_NAME_LENGTH);
        flag_set=3;			//SIGNAL FROM MONITOR
	buffer2_manager=0;
...
if(result1 == 1)
  {
         //printf("\nElasticity Action Successful");
         gtk_label_set_text(GTK_LABEL(elasticityresultlabel1),"Elasticity Engine: Elasticity Action Successful");
  }
  if(result1 == 2)
  {
           //printf("\nThis is minimum configuration");
           gtk_label_set_text(GTK_LABEL(elasticityresultlabel1),"Elasticity Engine: This is minimum configuration "); 
  }
  if(result1 == 3)
  {
          //printf("\nPlease Configure Nodes in cluster");
          gtk_label_set_text(GTK_LABEL(elasticityresultlabel1),"Elasticity Engine: Please Configure Nodes in cluster");
  }
  ...
  {
         //printf("\nEnough SUs in the Buffer");
         gtk_label_set_text(GTK_LABEL(buffer_actionlabel2),"Buffer status: Enough SUs in the Buffer"); 
  }
 if (buffer2_manager==2)
  {
         //printf("\nPlease Configure Nodes in cluster to manage futher workload chage");
         gtk_label_set_text(GTK_LABEL(buffer_actionlabel2),"Buffer Status: Please Configure Nodes in cluster to manage futher workload chage"); 
  }
...
int accessattribute1(SaImmHandleT immHandle,char *SUname,char attribute[SA_MAX_NAME_LENGTH])
{

 SaImmAccessorHandleT accessorHandle;
 int rc = EXIT_SUCCESS;
 SaImmAttrNameT  attributeNames[]={attribute,NULL};
 SaAisErrorT error;
 SaNameT objectName1;
...
void findSUsDraw(SaImmHandleT immHandle,SaUint8T *ch,int SGcounter)
{
 ...
 char objectstr[maxsize];
   strcpy(objectstr,"safSu=");
    ...
    searchParam.searchOneAttr.attrValue = NULL;
    SaNameT rootName = { 0, "" };
    strncpy((char *)rootName.value,ch, SA_MAX_NAME_LENGTH);
    rootName.length = strlen((char *)rootName.value);
    SaImmAttrNameT  attributeNames[] ={objectstr,NULL};
    SaImmAttrValuesT_2 **attributes;
    SaAisErrorT error;
    long int pos;
    char ch1[SA_MAX_NAME_LENGTH];
    int assignmentactive,assignmentstandby;
    //**SIs of SG1**//
    char attribute[SA_MAX_NAME_LENGTH];   
  
      error = saImmOmSearchInitialize_2(immHandle, &rootName, scope1, SA_IMM_SEARCH_GET_SOME_ATTR , &searchParam, attributeNames , &searchHandle);
    do {
                assignmentactive=0,assignmentstandby=0;
		error = saImmOmSearchNext_2(searchHandle, &objectName, &attributes);
                ...
                if (error == SA_AIS_OK)
                {
                    ...
                    if(ch == NULL)
                      ...
                      else
                      {
                          ...
                          if(pos == 0)
                          {
				 strncpy(attribute,"saAmfSUNumCurrActiveSIs",SA_MAX_NAME_LENGTH);
				 assignmentactive = accessattribute1(immHandle,objectName.value,attribute);
				 strncpy(attribute,"saAmfSUNumCurrStandbySIs",SA_MAX_NAME_LENGTH);
                                 assignmentstandby = accessattribute1(immHandle,objectName.value,attribute);
				 sprintf(ch1,"%s\nActive :%d\nStandby :%d",ch,assignmentactive,assignmentstandby);
                             if(redrawSUs==1)
                               {
                                 gtk_object_set(GTK_OBJECT(buttonSU[SGcounter][SUcounter]), "label", ch1, NULL);
                               }
                              else
                               { 
				 buttonSU[SGcounter][SUcounter] = gtk_button_new_with_label(ch1);
				 gtk_widget_set_size_request(buttonSU[SGcounter][SUcounter], 340, 60);
                               
				 gtk_fixed_put(GTK_FIXED(fixed), buttonSU[SGcounter][SUcounter], 0+(SUcounter*340), 60+(SGcounter*60));
				}
                                 if(assignmentstandby>0 || assignmentactive>0)
                                 {
                                        gtk_widget_modify_bg (GTK_WIDGET(buttonSU[SGcounter][SUcounter]), GTK_STATE_NORMAL, &colorlist[SGcounter]);   
                                 }
                    ...
                    if(ch == NULL)
                      ...
                      else
                      {
                          ...
                          if(pos == 0)
                          {
                                 ...
                                 //SIcounter++;
                                 buttonSI[SIcounter][SIcounter] = gtk_button_new_with_label(ch);
				 gtk_widget_set_size_request(buttonSI[SIcounter][SIcounter], 300, 35);
				 gtk_fixed_put(GTK_FIXED(fixed), buttonSI[SIcounter][SIcounter], 0+(SIcounter*300), 500+(SGcounter*50));
                                 //printf("\n%s",ch);
 				 gtk_widget_modify_bg (GTK_WIDGET(buttonSI[SIcounter][SIcounter]), GTK_STATE_NORMAL, &colorlist[SGcounter]);
                                 //**click_event**//
                                 //gtk_signal_connect(GTK_OBJECT(button[SIcounter][SIcounter]),"clicked",GTK_SIGNAL_FUNC(callback),(int *)1); 
				 gtk_widget_add_events(buttonSI[SIcounter][SIcounter],GDK_BUTTON_PRESS_MASK);
                                 gtk_signal_connect(GTK_OBJECT(buttonSI[SIcounter][SIcounter]),"button-press-event",G_CALLBACK(button_press),(int *)1);                                 
		...
		if (error == SA_AIS_OK)
                {
                      ...
                      {   
                        ...
                        strncpy(assclass,object.value,SA_MAX_NAME_LENGTH);
...
SaAisErrorT get_attrValueType(const SaImmClassNameT className,	SaImmAttrNameT attrName, SaImmValueTypeT *attrValueType)
{
	SaAisErrorT rc;
...
int change_rank(SaImmHandleT immHandle,SaNameT objectName,int SURank,char *argv[])
{
  printf("\nchange_rank");
...
SaAisErrorT error;
char assclass[SA_MAX_NAME_LENGTH],pointer1[SA_MAX_NAME_LENGTH];
SaImmScopeT scope = SA_IMM_SUBTREE;	/* default search scope */
int flag=1,flag1=1;
SaUint32T noassgSI;
 int objectNames_len = 1;
...
int create_listSGSU(SaImmHandleT immHandle,SaNameT objectnameSG1,SGSU listSGSU[])
{
 ...
 // printf("\n%s",objectnameSG1.value);
  SaImmSearchHandleT searchHandle100;
  SaNameT rootName = { 0, "" };
...
error = saImmOmSearchInitialize_2(immHandle, &rootName, scope1, SA_IMM_SEARCH_GET_SOME_ATTR , &searchParam, attributeNames , &searchHandle100);
...
if (SA_AIS_OK == error) {
	...
	do {
		error = saImmOmSearchNext_2(searchHandle100, &objectName, &attributes);
		if (error != SA_AIS_OK && error != SA_AIS_ERR_NOT_EXIST) {
			printf("error - saImmOmSearchNext_2 FAILED: %d\n", error);
			exit(EXIT_FAILURE);
		}
...
error = saImmOmSearchFinalize(searchHandle100);
...
int Rankchange_npm(SaImmHandleT immHandle,SaNameT objectnameSG1,char *argv[],int no_SUs,SGSU listSGSU[15])
{
//printf("\nThe ranking");

SaAisErrorT error;
int i,flag=1,SURank,rc = EXIT_SUCCESS,rc1,cluster_level=0;
...
if(flag==0)
{
	for(i=0;i<no_SUs;i++)
	{
		rc1 = change_rank(immHandle,listSGSU[i].SUName,listSGSU[i].rank,argv);
                if(rc1==1)
                {
                   return 0;   
                }
...
///search number of SIs_start
  printf("Calculating the total required capacity i.e ");
        							    searchParam1.searchOneAttr.attrName = "saAmfSIProtectedbySG";
 								    searchParam1.searchOneAttr.attrValueType = SA_IMM_ATTR_SANAMET;
 								    searchParam1.searchOneAttr.attrValue = &objectnameSG1;
...
change_rankSI_increase(immHandle,listSGSU,no_SUs);
   ...
   clusterlevel= Rankchange_npm(immHandle,objectnameSG1,argv,no_SUs,listSGSU);
if(clusterlevel==1)
{
...
SaNameT rootName = { 0, "" };
 ...
 SaNameT **objectNames = NULL;
  ...
  char *s;
 char nameccbattribute[256],noassg[256];
 char assclass[SA_MAX_NAME_LENGTH],pointer1[SA_MAX_NAME_LENGTH],pointer2[SA_MAX_NAME_LENGTH];
 SaImmAdminOwnerNameT adminOwnerName = create_adminOwnerName(basename(argv[0]));
...
search(immHandle,objectnameSI,assclass,pointer2); 
strncpy((char *)objectsity.value, pointer2, SA_MAX_NAME_LENGTH);
   	objectsity.length= strlen(objectsity.value);
   	printf("The SI .. type is %s \n",objectsity.value);
...
char assclass[SA_MAX_NAME_LENGTH],pointer1[SA_MAX_NAME_LENGTH],pointer2[SA_MAX_NAME_LENGTH];
 char nameccbattribute[256],noassg[256];
SGSU listSGSU[15];
int no_SUs,i,change_rank_sucess=0;
 SaImmAdminOwnerNameT adminOwnerName = create_adminOwnerName(basename(argv[0]));
...
*/
 ...
 search(immHandle,objectnameSI,assclass,pointer1); 
   ...
   strncpy((char *)objectsity.value, pointer1, SA_MAX_NAME_LENGTH);
   objectsity.length= strlen(pointer1);
   printf("\nThe objectsity .. type is %s \n",objectsity.value);
                                                                        ...
                                                                        if (error !=SA_AIS_OK)
 									{
										...
										maxactSIsperSU = *((SaUint32T *)attrValue);
               						   			error = saImmOmAccessorFinalize(accessorHandle);
                                   						...
                                   						printf("The saAmfCtDefNumMaxStandbyCSIs of %s SG  are %d\n",objectnameSG1.value,maxactSIsperSU);
					               	                }
...
error = saImmOmSearchFinalize(searchHandle1);
  ...
  if(flag1 == 1)
  {
	...
	for(i=0;i<no_SUs;i++)
	{
		rc = change_rank(immHandle,listSGSU[i].SUName,listSGSU[i].rank,argv);
		if(rc == 1)  //could not change rank
		{
	  		change_rank_sucess = 1;
		}
	}
	if(change_rank_sucess == 1)
	{
	   rc = 1;
	}
	else
...
SaAisErrorT error;
 ...
 int rc = EXIT_SUCCESS;
char *env_comp_name;
env_comp_name = "magic2";

const SaImmOiImplementerNameT implementerName = (SaImmOiImplementerNameT) "@safAmfService1";
...
int choice= (int) strtol(argv[2], (char **) NULL, 10);
char *SIname = argv[1];
char str[100];
int res;
/*
...
//GUI
printf("%c[1;32m\n**************Welcome to Elasticity Engine-e**********************************\n",27);
if(callbackflag !=1)
{
//gets(nameSI);
objectnameSI.length= strlen(SIname);
strncpy((char *)objectnameSI.value, SIname, SA_MAX_NAME_LENGTH);
//printf("shhh... SI name is :'%s' with choice %d \n", objectnameSI.value, choice);
fflush(stdout);
printf("\nName of SI: %s\n", objectnameSI.value);
printf("%c[0m",27);
...
elasticityact = choice;
printf("%c[1;32m\nElasticity action: %d (1 for increase 2 for decrease) :",27, elasticityact); 
printf("%c[0m",27);
